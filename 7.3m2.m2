    ...
    for j from 0 to numMini-1 do (
        if j!=i then ( --so are looking at different portals
            ...
            if potentialZero_0 and not(stoppingCrit()) then (
                ...
                iterateOnce(F, potentialZero_1, j, index);
            );
    ...
    for j from 0 to numMega-1 do(
        if i!=j then (
            ...
            for x0 in partialSols do (
                --THIS IS NOT THE CORRECT INDEX i
                if i==min(i,j) then (iterateOnce(parametrizedF, x0, 0, (min(i,j),max(i,j)));
                ) else (iterateOnce(parametrizedF, x0, 1, (min(i,j),max(i,j))););
...
numMini=10; --number of points to be in complex line rga case
